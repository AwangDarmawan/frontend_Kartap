...
//   const handleUbahClick = (id, decision) => {
//     setDataDiangkatBYID(id);
//     setDecision(decision);
//     if (decision) {
//       setModalDisetujui(true);  // Tampilkan modal Disetujui
//     } else {
//       setModalDitolak(true);  // Tampilkan modal Ditolak
//     }
//   };
...
//               {DataDiangkat.filter(item => item.keputusan_diangkat).map((item) => { 
...
//                           {getKriteriaName(kriteriaItem.kriteria)} : {getSubKriteriaName(kriteriaItem.hasil_evaluasi_faktor)}
//                         </div>
//                       ))}
//                     </td>
...
//                       {item.validasi_manager === true ? "Disetujui" : item.validasi_manager === false ? "Ditolak" : "Belum divalidasi"}
//                     </td>
//                     <td className="text-nama">{item.keterangan}</td>
//                     <td className="aksi-btn">
//                       <div className="btn-wrapper d-flex gap-2">
//                         <button className={`btn btn-create ${item.validasi_manager ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => handleUbahClick(item.id, true)}>
//                           Setujui
//                         </button>
//                         <button className={`btn btn-delete ${item.validasi_manager ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => handleUbahClick(item.id, false)}>
//                           Ditolak
//                         </button>
//                       </div>
//                     </td>
